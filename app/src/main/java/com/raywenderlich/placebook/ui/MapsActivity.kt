...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    //NOTE: moved argument out of parentheses may need to be moved back
    private fun createBookmarkObserver() {
        mapsViewModel.getBookmarkViews()?.observe(
            ...
            map.clear()
            markers.clear()
            it?.let { displayAllBookmarks(it); bookmarkListAdapter.setBookmarkData(it) }
        }
    }

    private fun displayAllBookmarks(bookmarks: List<MapsViewModel.BookmarkView>) { bookmarks.forEach { addPlaceMarker(it) } }

    private fun displayPoi(pointOfInterest: PointOfInterest) { showProgress(); displayPoiGetPlaceStep(pointOfInterest) }
